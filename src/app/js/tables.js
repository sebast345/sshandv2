...
function getPagination(table) {
  ...
  $('#maxRows').on('change', function(evt) {
     ...
     lastPage = 1;
      ...
      var totalRows = $(table + ' tr').length; // numbers of rows
      if (maxRows == 5000) {

        $('#pagination').hide();
        
      } else {
            if(maxRows >= totalRows) $('#pagination').hide();
      ...
      if (totalRows > maxRows) {
        ...
        for (var i = 1; i <= pagenum; ) {
          ...
          $('#pagination #prev')
            .before(
              '<li class="pag-item pointer centerFlex" data-page="' +
                i +
                '">\
								  <span>' +
                i++ +
      ...
      }else // end if row count > max rows
